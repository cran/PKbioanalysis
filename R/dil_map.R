...
.gen_graph <- function(df) {
  ...
  DiagrammeR::create_graph() |>
    ...
    DiagrammeR::mutate_node_attrs(
      label = gsub("(.*)_(.*)", "\\1@\\_\\{\\2\\}", .data$label)
    ) |> # sub location
    DiagrammeR::mutate_node_attrs(
      label = paste0(.data$label, '@^{', .data$id, '}')
    ) |> #
...
.parallel_dilution <- function(
  ...
  df <- df |>
    ...
    dplyr::mutate(
      v1 = paste0(fold * as.numeric(.data$conc) / .data$dil, unit)
    ) |>
    dplyr::mutate(
      v0 = paste0(
        as.numeric(.data$conc) / .data$dil,
        unit,
        "_",
